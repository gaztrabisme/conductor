...
Written for the coordinator composing a run: what "dynamic workflow" means here, independent of any harness, and how each harness binds the abstract. Per-harness facts (paths, caps, what stays MISSING) live in `references/harness-map.md`; read the row for the harness this run is in before selecting an engine.
...
- **Units to lanes.** Every unit already has a lane, a brief, and an acceptance command (`references/lane-plan.md`). The workflow is the schedule that connects them: which units start immediately, which wait on which.
...
- **Verify layer.** A stage whose brief instructs it to refute, not confirm (the task-force rule: `references/task-force-protocol.md`). Size it to the stakes: load-bearing claims get a verifier each; the tail gets sampling. Also size it to the harness's documented cap, and where no cap is documented, to what you can watch (the 154-agent lesson: `references/lane-plan.md` — a Claude workflow reached 154 agents before the session limit killed it with 7 agents done).
- **Converge.** The meeting point where the stages end and the threads reconcile into one artifact set: `references/convergence.md`. Declare it before dispatch, never after.
- **Resume.** Where the harness offers a run id or a re-runnable workflow list, plan on resuming a failed or interrupted run from its last completed stage instead of re-running completed work. Claude Code re-runs workflows from `/workflows`; Grok resumes sessions with `-r/--resume` and `-c/--continue` (`references/harness-map.md`). Where the harness offers nothing, the run ledger is the resume state: every completed unit with its verdict is a stage that does not run twice.
...
The map citation [map S#] resolves in `references/harness-map.md`'s Sources section, all fetched 2026-09-16.
